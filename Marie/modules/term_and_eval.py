...
from Marie import pbot, OWNER_ID
...
async def evaluate(client, message):
    status_message = await message.reply_text("`Running.....Wait`")
    try:
...
async def terminal(client, message):
    if len(message.text.split()) == 1:
        await message.reply("Usage: `/term echo ᴜᴡᴜ` ")
        return
...
""".format(
    ...
    if output:
        if len(output) > 4096:
            with open("Marie/output.txt", "w+") as file:
                file.write(output)
