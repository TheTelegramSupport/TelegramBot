...
from Marie.modules.sql import antiflood_sql as sql
...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        msg.reply_text(
            "I Can't Restrict People Here Give Me Permission First! Until Then, I'll Disable Anti-Flood.",
        )
...
def set_flood(update, context) -> str:
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(
                update.effective_message,
                "ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ɪꜱ ᴍᴇᴀɴᴛ ᴛᴏ ᴜꜱᴇ ɪɴ ɢʀᴏᴜᴘ ɴᴏᴛ ɪɴ ᴘᴍ ʙᴀʙʏ🥀 This Command Is Meant To User In Group Not In PM..lol ",
            )
            return ""
...
def set_flood_mode(update, context):
    ...
    Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks.""",
            ...
            send_message(
                update.effective_message,
                "I Only Understand ban/kick/mute/tban/tmute .Okay!",
            )
            return
        if conn:
            text = msg.reply_text(
                "Exceeding Consecutive Flood Limit Will Result In {} In {} okay.!".format(
                    settypeflood,
                    chat_name,
                ),
            )
    ...
    if conn:
        text = msg.reply_text(
            "Sending More Message Than Flood Limit Will Result In {} In {} .".format(
                settypeflood,
                chat_name,
            ),
        )
...
__help__ = """
Antiflood Allows You To Take Action On Users That Send More Than X Message In A Row. Exceeding The Set Flood Will Result In Restricting That User.
    This Will Mute Users If They Send More Than 10 Message In A Row, Bots Are Ignored.
     • `/flood`*:* Get The Current Flood Control Setting
    • *Admins Only:*
     • `/setflood <int/'no'/'off'>`*:* Enables Or Disable Flood Control 
     *Example:* `/setflood 10`
