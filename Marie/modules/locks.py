import Marie.modules.sql.locks_sql as sql
import html
import ast
...
from Marie import dispatcher, DRAGONS, LOGGER
...
def lock(update, context) -> str:
    ...
    if (
        ...
        send_message(
            update.effective_message,
            "I Am Not Admin Or Haven't Got Enough Rights.",
        )
            ...
            if lockable == "bots":
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        ...
                        send_message(
                            update.effective_message,
                            "Only Admins Are Allowed To Add Bots In This Cchat! Get Sutta Here .",
                        )
                        break
            else:
                try:
...
def build_lock_message(chat_id):
    ...
    if locklist:
        ...
        for x in locklist:
            res += "\n × {}".format(x)
    res += "\n\n*" + "These Are The Current Chat Permission:" + "*"
    for x in permslist:
        res += "\n × {}".format(x)
    return res
...
Telegram World; The Bot Will Automatically Deleted Them!
...
*Note:*
» Unlocking Permission *Info* ᴡɪʟʟ ᴀʟʟᴏᴡ ᴍᴇᴍʙᴇʀꜱ (Non-Admins) To Change The Group Information, Such As The Description Or The Group Name 
