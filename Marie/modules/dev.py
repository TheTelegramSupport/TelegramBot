...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not Marie.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        Marie.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Marie.ALLOW_CHATS = False
    else:
    ...
    if args:
        ...
        try:
            ...
            update.effective_message.reply_text(
                "Beep Boop, I Could Not Leave That Group (Dumpo Why Tho).",
            )
            return
