...
from Marie import ubot2
from Marie.events import register
from Marie import telethn as tbot, TEMP_DOWNLOAD_DIRECTORY, SUPPORT_CHAT


@register(pattern="^/kamuii ?(.*)")
async def _(fry):
    level = fry.pattern_match.group(1)
    kntl = await fry.reply("`Processing... `")
    if fry.fwd_from:
        return
    ...
    if reply_message.sender.bot:
        await fry.edit("`Reply To A Sticker To Destroy`")
        return
