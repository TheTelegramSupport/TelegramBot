...
from Marie import dispatcher, telethn
from Marie.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("Marie.modules." + text)
    except:
    ...
    load_messasge.edit_text(
        "ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇ : <b>{}</b>".format(text),
        parse_mode=ParseMode.HTML,
    )
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Marie.modules.", 1)[1]
