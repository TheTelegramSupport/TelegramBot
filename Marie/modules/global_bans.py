...
import Marie.modules.sql.global_bans_sql as sql
from Marie.modules.sql.users_sql import get_user_com_chats
from Marie import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
from Marie.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEMONS:
        message.reply_text(
            "Ooh Someone's Trying To Gban A Demon Disaster! *Grabs Popcorn*",
        )
        return
    ...
    if user_id == bot.id:
        message.reply_text("-You Uhh...Want Me To Puch Myself?")
        return
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if old_reason:
            message.reply_text(
                "This User already Gbanned, For The Following Reason:\n"
                "<code>{}</code>\n"
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                message.reply_text(f"Could Not Gban Due To: {excp.message} Sorry!.")
                if EVENT_LOGS:
                    bot.send_message(
                        EVENT_LOGS,
                        f"Could Not Gban Due To {excp.message} Sorry!.",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    send_to_list(
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in UNGBAN_ERRORS:
                ...
                message.reply_text(f"Could Not Un－Gabn Due To: {excp.message} Sorry!")
                if EVENT_LOGS:
                    bot.send_message(
                        EVENT_LOGS,
                        f"Could Not Un－Gabn Due To: {excp.message} Sorry!",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    bot.send_message(
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    with BytesIO(str.encode(banfile)) as output:
        ...
        update.effective_message.reply_document(
            ...
            caption="Here Is The List Of Currently Gbanned User.",
        )
...
def gbanstat(update: Update, context: CallbackContext):
    ...
    if len(args) > 0:
        ...
        update.effective_message.reply_text(
            ...
            "When False, That Won't Leaving You At The Possible Mercy Of"
