...
import Marie.modules.sql.blsticker_sql as sql
from Marie import LOGGER, dispatcher
    ...
    for text in split_text:
        if (
            ...
            send_message(
                ...
                "There Are No Blacklist Sticker In <b>{}</b>!".format(
...
def add_blackliststicker(update: Update, context: CallbackContext):
    ...
    if len(words) > 1:
        ...
        send_message(
            update.effective_message,
            "Sticker <code>{}</code> Added To Blacklist Sticker In <b>{}</b>...!".format(
                trigger,
                html.escape(chat_name),
            ),
            parse_mode=ParseMode.HTML,
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                send_message(
                    update.effective_message,
                    "Stickers <code>{}</code> Deleted From Blacklist In <b>{}</b>..!".format(
            ...
            send_message(
                update.effective_message,
                "Stickers <code>{}</code> Deleted From Blacklist  {} Did Not Exist, So It's Not Deleted.".format(
                    successful,
        ...
        if success:
            send_message(
                update.effective_message,
                "Stickers <code>{}</code> Deleted From Blacklist  <b>{}</b> .!".format(
                    trigger,
                    chat_name,
                ),
                parse_mode=ParseMode.HTML,
...
def blacklist_mode(update: Update, context: CallbackContext):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(
                update.effective_message,
                " You Can Do This Command In Groups, Not PM..Lol",
            )
            return ""
    ...
    if args:
        ...
        return (
            "<b>{}:</b>\n"
            "<b>Admin:</b> {}\n"
            "Blacklist Sticker Mode Changed, Users Will Be {}....".format(
    ...
    if conn:
        ...
        text = "Blacklist Sticker Mode Is Currently Set To *{}* ʙᴀʙʏ🥀.".format(
            settypeblacklist,
        )
            ...
            try:
                ...
                if getmode == 1:
                    ...
                    warn(
                        ...
                        chat,
                        "Using Sticker '{}' Which In Blacklist Sticker".format(
                            trigger,
                        ),
                        message,
                        update.effective_user,
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} Muted Because Using '{}' Which In Blacklist Sticker".format(
                    ...
                    if res:
                        bot.sendMessage(
                            chat.id,
                            "{} Kicked Because Using '{}' Which In Blacklist Sticker".format(
                                mention_markdown(user.id, user.first_name),
                                trigger,
                            ),
                            parse_mode="markdown",
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} Banned Because Using '{}' Which In Blacklist Sticker".format(
                            mention_markdown(user.id, user.first_name),
                            trigger,
                        ),
                        parse_mode="markdown",
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} Banned For {} Because Using '{}' Which In Blacklist Sticker".format(
                            mention_markdown(user.id, user.first_name),
                            value,
                            trigger,
                        ),
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} Muted For {} Because Using '{}' Which In Blacklist Sticker".format(
                            mention_markdown(user.id, user.first_name),
                            value,
                            trigger,
                        ),
...
def __stats__():
    return "× {} Blacklist Sticker, Across {} Chats.".format(
