...
from Marie import BOT_NAME
from Marie import telethn as tbot
from Marie.events import register
from Marie.modules.sql.night_mode_sql import (
    add_nightmode,
    get_all_chat_id,
    is_nightmode_indb,
    rmnightmode,
...
async def close_ws(event):
    ...
    if not event.is_group:
        await event.reply("You Can Only Enable Night Mode In Group.")
        return
    if is_nightmode_indb(str(event.chat_id)):
        await event.reply("This Chat Has Already Enable Night Mode.")
        return
...
async def disable_ws(event):
    ...
    if not event.is_group:
        await event.reply("You Can Only Disable Night Mode In Group.")
        return
    if not is_nightmode_indb(str(event.chat_id)):
        await event.reply("This Chat Is Has Not Enabled Night Node.")
        return
    rmnightmode(str(event.chat_id))
    await event.reply(
        f"Removed Chats {event.chat.title} With ID {event.chat_id} From Database."
    )
...
__help__ = """
 » `/nightmode`*:* Add Group From Night Mode Chats
 » `/rmnight`*:* Removes Group From Night Mode Chats
