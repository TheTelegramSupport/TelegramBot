...
import Marie.modules.sql.welcome_sql as sql
from Marie import (
    DEV_USERS,
    OWNER_ID,
    DRAGONS,
...
from Marie.modules.helper_funcs.msg_types import get_welcome_type
        ...
        if excp.message == "Button_url_invalid":
            ...
            msg = update.effective_chat.send_message(
                markdown_parser(
                    (
                        backup_message
                        + "\nThe Current Message Has Buttons Which Use URL Protocols That Are Unsupported By Telegram. Please Update."
                    )
                ),
                parse_mode=ParseMode.MARKDOWN,
                reply_to_message_id=reply,
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        update.effective_message.reply_text(
            ...
            f"*The Welcome message (Not Filling The {{}}) ɪꜱ:*",
            parse_mode=ParseMode.MARKDOWN,
        )
...
def goodbye(update: Update, context: CallbackContext):
    ...
    if not args or args[0] == "noformat":
        ...
        update.effective_message.reply_text(
            ...
            f"*The Goodbye Message ( Not Filling The  {{}}) ɪꜱ:*",
            parse_mode=ParseMode.MARKDOWN,
        )
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                "I Understand 'On/Yes' ᴏʀ 'Off/No' Only!"
            )
...
def welcomemute(update: Update, context: CallbackContext) -> str:
    ...
    if len(args) >= 1:
        ...
        if args[0].lower() in ["soft"]:
            ...
            msg.reply_text(
                "I Will Restrict User's Permission To Send Mesia For 24 Hours."
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"#Welcome_Mute\n"
                f"<b>•Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Has Toggled Welcome Mute To <b>SOFT</b> ʙᴀʙʏ🥀."
            )
        ...
        if args[0].lower() in ["captcha"]:
            ...
            msg.reply_text(
                "I Will Now Mute People When They Join Until They prove They're Not A Bot .\nThey Have To Solved A Captcha To Get Unmuted."
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"#Welcome_Mute\n"
...
def clean_welcome(update: Update, context: CallbackContext) -> str:
    ...
    if not args:
        ...
        if clean_pref:
            update.effective_message.reply_text(
                "I Should Be Deleting Welcome Message Up To Two Days Old."
            )
        else:
            update.effective_message.reply_text(
                "I'm Currently Not Deleting Old Welcome Message!"
            )
        return ""

    if args[0].lower() in ("on", "yes"):
        sql.set_clean_welcome(str(chat.id), True)
        update.effective_message.reply_text("I'll Try To Delete Old Welcome Message!")
        return (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#CLEAN_WELCOME\n"
            f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
            f"Has Toggled Clean Welcome To <code>ON</code> ."
        )
    if args[0].lower() in ("off", "no"):
        ...
        return (
            ...
            f"#CLEAN_WELCOME\n"
            f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
            f"Has Toggled Clean Welcome To <code>OFF</code>."
        )
    update.effective_message.reply_text("I Understand 'On/Yes' ᴏʀ 'Off/No' Only!")
    return ""
...
def user_button(update: Update, context: CallbackContext):
    ...
    if join_user == user.id:
        ...
        query.answer(text="Yo! Your Are A Human, Unmuted!")
        bot.restrict_chat_member(
            chat.id,
            user.id,
            permissions=ChatPermissions(
...
def user_captcha_button(update: Update, context: CallbackContext):
    ...
    if join_user == user.id:
        ...
        if c_captcha_ans == captcha_ans:
            ...
            member_dict = VERIFIED_USER_WAITLIST[(chat.id, user.id)]
            member_dict["status"] = True
            query.answer(text="Yoyo! Your Are A Human, Unmuted!")
            bot.restrict_chat_member(
                chat.id,
                user.id,
                permissions=ChatPermissions(
            ...
            try:
                ...
                pass
            kicked_msg = f"""
            ❌ [{escape_markdown(join_usr_data.first_name)}](tg://user?id={join_user}) Failed The Captcha Nad Was Kicked.
            """
...
"You Can Get The Bot To Mute New People Who Join Your Group And Hence Present SpamBots From Flooding You Groups." 
   "The Following Options Are Possible:\ɴ"  
     "  • `/welcomemute soft`*:* Restricts New Members From Sending Media For 24 Hours.\ɴ"  
       "  • `/welcomemute strong`*:* Mutes New Members Till They Tap On A Button Thereby Verifying They're Human.\ɴ"  
         "  • `/welcomemute captcha`*:*  Mutes New Members Till They Solve A Button Captcha Thereby Verifying They're Human .\ɴ"   
          "  • `/welcomemute off`*:* Turns Off Welcome Mute.\ɴ"   
           "*Note:* Strong Mode Kicks A User From In 120Seconds. They Can Always Rejoin Though"
)
...
» `/welcome` Noformate*:* Show Current Welcome Settings, Without The Formatting - Useful To Recycle Your Welcome Messages!
...
» `/cleanservice` <On/Off>*:* Deletes Telegrams Welcome/Left Service Message. *Example:*User Joined Chat, User Left Chat.

*Welcome Markdown:*
» `/welcomehelp`*:* View More Formatting Information For Custom Welcome/GoodBye Message
"""
