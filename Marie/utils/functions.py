...
from Marie import aiohttpsession as aiosession
from Marie.services.dbfunctions import start_restart_stage
from Marie.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "Marie"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("Marie/MarieFont/logofont", 55)
    file = f"Marie/MarieFont/{randint(1000, 9999)}.jpg"
